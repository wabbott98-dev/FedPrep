...
function RadarChart({ scores }) {
  ...
  const pts = dims.map((_,i)=>{
    ...
    const v=scores?Object.values(scores)[i]||0:0;
    return {x:cx+(v/5)*r*Math.cos(a),y:cy+(v/5)*r*Math.sin(a)};
  });
...
function AgencyBadge({ name }) {
  return (
    <div style={{padding:"6px 14px",background:"#0a1628",border:"1px solid #1e3a5f",borderRadius:20,fontSize:11,color:"#64748b",whiteSpace:"nowrap"}}>
      {name}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    if (sessions.length) localStorage.setItem("fedprep_sessions", JSON.stringify(sessions));
  }, [sessions]);
  ...
  const saveProfile = () => {
    localStorage.setItem("fedprep_profile",JSON.stringify(profile));
    setScreen("dashboard");
  };
  ...
  const handleUpgrade = (tier) => {
    ...
    setUser(updated); localStorage.setItem("fedprep_user",JSON.stringify(updated));
    setScreen("dashboard"); setUpgradeTarget(null);
  };
  ...
  const startPractice = (q) => {
    ...
    setSelectedQ(q); setResponse(""); setFeedback(null); setTimer(0); setTimerActive(true); setScreen("practice");
  };
...
Return ONLY: {"scores":{"structure":0,"relevance":0,"specificity":0,"competency_alignment":0,"professionalism":0},"total":0,"strengths":[],"improvements":[],"suggested_answer":"","flags":[],"next_tip":""}`;
    try {
      ...
      setFeedback(parsed);
      setSessions(prev=>[...prev,{question:selectedQ,response,feedback:parsed,time:timer,date:new Date().toLocaleDateString()}]);
      setScreen("feedback");
    } catch(e) {
    ...
    <div style={{...S.page, overflowX:"hidden"}}>
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,background:"#020b18ee",backdropFilter:"blur(12px)",borderBottom:"1px solid #1e3a5f22",padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <button onClick={()=>{setAuthMode("signin");setScreen("auth");}}
            style={{padding:"7px 16px",background:"transparent",border:"1px solid #1e3a5f",borderRadius:8,color:"#94a3b8",fontSize:12,cursor:"pointer"}}>
            Sign In
          </button>
          <button onClick={()=>{setAuthMode("signup");setScreen("auth");}}
            style={{padding:"7px 16px",background:"linear-gradient(135deg,#1a5c9e,#4a9eff)",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>
            Start Free
          </button>
        </div>
      </nav>
      ...
      <section style={{padding:"60px 20px 50px",maxWidth:640,margin:"0 auto",textAlign:"center",position:"relative"}}>
        ...
        <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:58,color:"#f1f5f9",letterSpacing:3,margin:"0 0 8px",lineHeight:1}}>
          ACE YOUR<br/>
          <span style={{color:"#4a9eff"}}>FEDERAL</span> ORAL BOARD
        </h1>

        <p style={{fontSize:15,color:"#64748b",lineHeight:1.7,margin:"16px auto 32px",maxWidth:480}}>
          Most candidates walk into federal panel interviews unprepared. Our AI-powered coaching platform trains you the way real hiring panels evaluate — question by question, response by response.
        </p>

        <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap",marginBottom:40}}>
          <button onClick={()=>{setAuthMode("signup");setScreen("auth");}}
            style={{padding:"14px 32px",background:"linear-gradient(135deg,#1a5c9e,#4a9eff)",border:"none",borderRadius:10,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",letterSpacing:1}}>
            START FREE — NO CARD NEEDED →
          </button>
          <button onClick={()=>{setAuthMode("signin");setScreen("auth");}}
            style={{padding:"14px 24px",background:"transparent",border:"1px solid #1e3a5f",borderRadius:10,color:"#94a3b8",fontSize:13,cursor:"pointer"}}>
            Sign In
          </button>
        </div>
      ...
      <section style={{padding:"0 20px 40px",maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center"}}>
          {["CBP","USDA / APHIS","Border Patrol","ICE / HSI","DEA","ATF","FBI","Secret Service","USMS","TSA"].map(a=>(
            <AgencyBadge key={a} name={a}/>
          ))}
        </div>
      </section>
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:16,marginBottom:28}}>
            ...
            <div>
              ...
              <p style={{margin:0,fontSize:13,color:"#64748b",lineHeight:1.7}}>
                With 17 years as a CBP Agriculture Specialist, I've watched qualified candidates fail federal oral boards — not because they weren't capable, but because they didn't know how panels actually evaluate responses. FedPrep closes that gap.
              </p>
            </div>
          </div>
      ...
      <section style={{padding:"48px 20px",maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <FeatureCard icon="🎯" title="Role-Specific Questions" desc="Questions built for CBP, USDA, Border Patrol, ICE, DEA and more — not generic HR questions."/>
          <FeatureCard icon="🤖" title="AI Scoring Engine" desc="Instant feedback across 5 dimensions with a 25-point panel rubric used by real hiring boards."/>
          <FeatureCard icon="⚠️" title="Panel Flag Detection" desc="AI catches vague language, missing STAR components, and blame language before the real panel does."/>
          <FeatureCard icon="📈" title="Progress Tracking" desc="Radar chart, session history, and competency trends show exactly where you're growing."/>
          <FeatureCard icon="💬" title="Stronger Answer Examples" desc="Every feedback session includes a sample response showing what a panel-ready answer looks like."/>
          <FeatureCard icon="⏱️" title="Timed Practice" desc="Built-in timer simulates real panel pressure so you're never caught off guard by pacing."/>
        </div>
      </section>
      ...
      <section style={{padding:"0 20px 48px",maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <TestimonialCard
            quote="I'd failed two CBP oral boards before using FedPrep. The AI feedback showed me I was using 'we did' instead of owning my actions. Fixed that one habit and passed on my third attempt."
            name="Marcus T." role="CBP Officer Candidate · Tampa, FL"/>
          <TestimonialCard
            quote="The STAR format breakdown is exactly what was missing from my prep. I didn't realize panels scored structure separately from content. Game changer."
            name="Denise R." role="Border Patrol Agent Candidate · El Paso, TX"/>
          <TestimonialCard
            quote="The technical questions for agriculture and import inspection are unlike anything else out there. Clearly built by someone who knows the job."
            name="James K." role="Agriculture Specialist Candidate · Miami, FL"/>
        </div>
      </section>
      ...
      <section style={{padding:"0 20px 48px",maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{background:"#0a1628",border:"1px solid #1e3a5f",borderRadius:16,padding:20,marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:"#64748b",letterSpacing:1}}>Free</div>
              <div style={{fontSize:11,color:"#334155"}}>No credit card required</div>
            </div>
          ...
          <button onClick={()=>{setAuthMode("signup");setScreen("auth");}} style={{...S.btnPrimary,marginTop:14,background:"#0f1f33",color:"#64748b",fontSize:14}}>
            GET STARTED FREE →
          </button>
        </div>
        ...
        {["monthly","federal_pack","coaching"].map(tierKey=>{
          ...
          const featured = tierKey==="federal_pack";
          return (
            <div key={tierKey} style={{background:"#0a1628",border:`2px solid ${featured?t.color:"#1e3a5f"}`,borderRadius:16,padding:20,marginBottom:12,position:"relative",overflow:"hidden"}}>
              ...
              {featured && (
                <div style={{position:"absolute",top:12,right:12,fontSize:10,padding:"3px 10px",background:`${t.color}22`,color:t.color,borderRadius:20,fontWeight:700,letterSpacing:1}}>MOST POPULAR</div>
              )}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                <div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:t.color,letterSpacing:1}}>{t.label}</div>
                  <div style={{fontSize:11,color:"#475569"}}>{t.questions} questions unlocked</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:"#f1f5f9",letterSpacing:1}}>{t.price}</div>
                  <div style={{fontSize:11,color:"#64748b"}}>{t.period}</div>
                </div>
              </div>
              ...
              <button onClick={()=>{setAuthMode("signup");setScreen("auth");}}
                ...
                {tierKey==="federal_pack"?"BUY ONE-TIME →":"SUBSCRIBE →"}
              </button>
            </div>
          );
        })}
      ...
      <section style={{padding:"0 20px 60px",maxWidth:640,margin:"0 auto"}}>
        <div style={{background:"linear-gradient(135deg,#0a1628,#0f1f33)",border:"1px solid #1e3a5f",borderRadius:20,padding:"40px 28px",textAlign:"center",position:"relative",overflow:"hidden"}}>
          ...
          <h2 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:42,color:"#f1f5f9",letterSpacing:2,margin:"0 0 12px",lineHeight:1}}>
            DON'T WALK IN<br/>UNPREPARED
          </h2>
          <p style={{fontSize:13,color:"#64748b",margin:"0 0 28px",lineHeight:1.7}}>
            Start with 3 free questions today. No credit card. No commitment.<br/>Just you, the AI, and the preparation you need.
          </p>
          <button onClick={()=>{setAuthMode("signup");setScreen("auth");}}
            style={{padding:"16px 40px",background:"linear-gradient(135deg,#1a5c9e,#4a9eff)",border:"none",borderRadius:12,color:"#fff",fontSize:18,fontWeight:700,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",letterSpacing:2}}>
            START FREE NOW →
          </button>
        </div>
      </section>
  ...
  if (screen === "auth") return (
    <div style={{...S.page,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:440}}>
        ...
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:44,color:"#f1f5f9",letterSpacing:3,margin:"0 0 4px",lineHeight:1}}>
            {authMode==="signup"?"JOIN FED PREP":"WELCOME BACK"}
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,background:"#0f1f33",borderRadius:10,padding:4,marginBottom:22}}>
            {["signin","signup"].map(m=>(
              <button key={m} onClick={()=>{setAuthMode(m);setAuthError("");}}
                style={{padding:"9px",borderRadius:8,border:"none",background:authMode===m?"#1a3a5c":"transparent",color:authMode===m?"#4a9eff":"#64748b",fontSize:13,fontWeight:600,cursor:"pointer"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:20}}>
            {["email","google"].map(m=>(
              <button key={m} onClick={()=>setAuthMethod(m)}
                style={{padding:"10px",borderRadius:8,border:`1px solid ${authMethod===m?"#4a9eff":"#1e3a5f"}`,background:authMethod===m?"#1a3a5c":"transparent",color:authMethod===m?"#4a9eff":"#64748b",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          ...
          {authMethod==="google" ? (
            <div>
              <button onClick={handleGoogleAuth}
                style={{...S.btnPrimary,background:"#fff",color:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontSize:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {["Entry","Experienced","Supervisory"].map(l=>(
                <button key={l} onClick={()=>setProfile(p=>({...p,level:l}))}
                  style={{padding:"10px 8px",borderRadius:8,border:`1px solid ${profile.level===l?"#4a9eff":"#1e3a5f"}`,background:profile.level===l?"#1a3a5c":"#0f1f33",color:profile.level===l?"#4a9eff":"#64748b",fontSize:12,cursor:"pointer"}}>
                  {l}
                </button>
              ))}
            </div>
          ...
          return (
            <div key={tierKey} style={{background:"#0a1628",border:`2px solid ${isTarget?t.color:"#1e3a5f"}`,borderRadius:16,padding:20,marginBottom:12,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                ...
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:"#f1f5f9"}}>{t.price}</div>
                  <div style={{fontSize:11,color:"#64748b"}}>{t.period}</div>
                </div>
              </div>
              {t.features.map((f,i)=><div key={i} style={{fontSize:12,color:"#94a3b8",marginBottom:5,display:"flex",gap:8}}><span style={{color:t.color}}>✓</span>{f}</div>)}
              {isCurrent ? (
                ...
                <button onClick={()=>handleUpgrade(tierKey)}
                  style={{width:"100%",marginTop:14,padding:"12px",background:`linear-gradient(135deg,${t.color}88,${t.color})`,border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",letterSpacing:1}}>
                  {tierKey==="federal_pack"?"BUY ONE-TIME ACCESS →":"SUBSCRIBE NOW →"}
                </button>
              )}
            </div>
          );
      ...
      <div style={S.header}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <button onClick={()=>setScreen("upgrade")}
            style={{padding:"5px 12px",background:`${tierInfo.color}22`,border:`1px solid ${tierInfo.color}55`,borderRadius:20,color:tierInfo.color,fontSize:11,cursor:"pointer",fontWeight:600}}>
            {tierInfo.label} ↑
          </button>
      ...
      <div style={{padding:20,maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:18}}>
          {[{label:"Sessions",value:sessions.length,color:"#4a9eff"},
            ...
            {label:"Panel Status",value:avgScore>=21?"Ready ✓":avgScore>=15?"Building":sessions.length?"Training":"New",color:avgScore>=21?"#22c55e":"#f59e0b"}].map(s=>(
        ...
        {user?.tier==="free" && (
          <button onClick={()=>setScreen("upgrade")}
            style={{width:"100%",marginBottom:18,padding:"13px 16px",background:"linear-gradient(135deg,#1a3a5c,#1e4a7a)",border:"1px solid #4a9eff44",borderRadius:12,color:"#f1f5f9",fontSize:13,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {categories.map(c=>(
              <button key={c} onClick={()=>setFilterCat(c)}
                style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${filterCat===c?"#4a9eff":"#1e3a5f"}`,background:filterCat===c?"#1a3a5c":"transparent",color:filterCat===c?"#4a9eff":"#64748b",fontSize:11,cursor:"pointer"}}>
                {c}
              </button>
            ))}
          </div>
          ...
          {filteredQs.map(q=>{
            ...
            return (
              <div key={q.id} onClick={()=>startPractice(q)}
                style={{background:locked?"#080f1a":"#0a1628",border:`1px solid ${done?"#1e5c3a":locked?"#0f1f33":"#1e3a5f"}`,borderLeft:`3px solid ${locked?"#1e3a5f":colors.accent}`,borderRadius:12,padding:16,cursor:"pointer",opacity:locked?0.7:1,position:"relative",overflow:"hidden"}}>
  ...
  if (screen === "practice") return (
    <div style={S.page}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;700&family=Bebas+Neue&display=swap" rel="stylesheet"/>
      <div style={S.header}>
        <button onClick={()=>{setTimerActive(false);setScreen("dashboard");}} style={{background:"none",border:"none",color:"#64748b",cursor:"pointer",fontSize:13}}>← Back</button>
      ...
      <div style={{padding:20,maxWidth:640,margin:"0 auto"}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <textarea value={response} onChange={e=>setResponse(e.target.value)}
            placeholder="Type your response as you would speak in a real panel interview..."
            rows={8}
        ...
        <button onClick={submitAnswer} disabled={loading||!response.trim()}
          ...
          {loading?"EVALUATING RESPONSE...":"SUBMIT FOR AI EVALUATION →"}
        </button>
      </div>
    </div>
  );
        ...
        {feedback?.error ? (
          ...
          <>
            <div style={{...S.card,textAlign:"center"}}>
              ...
              <span style={{fontSize:12,padding:"5px 14px",borderRadius:20,background:getRating(feedback.total).color+"22",color:getRating(feedback.total).color,fontWeight:700,letterSpacing:1}}>
                {getRating(feedback.total).label.toUpperCase()}
              </span>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <button onClick={()=>setScreen("dashboard")}
                style={{padding:"13px",background:"linear-gradient(135deg,#1a5c9e,#4a9eff)",border:"none",borderRadius:10,color:"#fff",fontSize:13,cursor:"pointer",fontWeight:600}}>
                Next Question →
              </button>
            </div>
          </>
